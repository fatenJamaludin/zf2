...
class RegistrationController extends AbstractActionController
{
	...
	public function generatePassword($l = 8, $c = 0, $n = 0, $s = 0) {
		 ...
		 // create arrays if special character required
		 if($count) {

			  $tmp1 = str_split($out);
			  $tmp2 = array();
	 
			  // add required special character to second array
			  for($i = 0; $i < $c; $i++) {
	...
	public function sendPasswordByEmail($usr_email, $password)
	{
		...
		$message->addTo($usr_email)
				...
				->setBody("Your password at  " . 
					...
					$password //password from generatePassword()
				);
		$transport->send($message);		
	}	
}